    ...
    if inte:
        while True:
            ...
            if positive and user_input <= 0:
                print(error_message, sep="")  # noqa: FURB105
                continue
...
def GetTimer():
    ...
    print(
        "- Type the format of time followed by the number that you want to increment the timer on. The formats are the following:"
    )
    print("[S]: seconds, [M]: minutes, [H]: hours.")
    print(
        "- It should be like: [S 30] to increment 30 seconds on the timer, or S -10 if you want do decrement. Type [end] to stop.\n"
    )
    while True:
...
while True:
    ...
    if (
        user_choice not in ["ADD", "LIST", "LOAD", "SAVE"]
        and task_lists[actual_list].total_tasks == 0
    ):
        AttentionMessage(
            "- You have no tasks in your list yet. Try start by typing [add]"
        )
        continue
            ...
            while True:
                ...
                match add_choice:
                    case "S":
                        ...
                        AttentionMessage(
                            f'- The Single Task "{task_name}" was added to the list.'
                        )
                    ...
                    case "C":
                        ...
                        task_name = UserInput(upper=False)
                        TakMessage("add s C")
                        task_complement = UserInput(
                            inte=True,
                            error_message="\n- Please, type a positive integer number",
                        )
                        task_lists[actual_list].AddTask(
                            pb.Task("C", task_name, task_complement)
                        )
                        AttentionMessage(
                            f'- The Checklist Task "{task_name}" was added to the task list.'
                        )
                    ...
                    case "T":
                        ...
                        task_lists[actual_list].AddTask(
                            pb.Task("T", task_name, task_complement)
                        )
                        AttentionMessage(
                            f'- The Timed Task "{task_name}" was added to the task list.'
                        )
                    ...
                    case _:
                        AttentionMessage(
                            "- I don't recognize this type. Please, choice something between: [S], [C] or [T]",
                            True,
                        )
                        print()
                        continue
                ...
                if done_choice[0] != "-":
                    ...
                    match task.type:
                        case "S":
                            ...
                            print()
                            AttentionMessage(
                                f"- The task: {task.name} was marked as done."
                            )
                            print()
                            ShowTasksInList()
                        case "C":
                            ...
                            while True:
                                checks = UserInput(
                                    positive=True,
                                    inte=True,
                                    error_message="- Please, type a positive integer number.",
                                )
                                if task.checks + checks > task.complement:
                                    AttentionMessage(
                                        "- That extrapolates the max number of checks to mark.",
                                        True,
                                    )
                                    print()
                                    continue
    ...
    Type [Y] if so, else type anything.""")
                    ...
                    if not task.done:
                        AttentionMessage(
                            "That task is already set as unfinished.", True
                        )
                        continue
                    ...
                    match task.type:
                        case "C":
                            print()
                            print(
                                "- Ok, now, type the number of checks you want to remove"
                            )
                            while True:
                                checks = UserInput(
                                    positive=True,
                                ...
                                if task.checks - checks < 0:
                                    AttentionMessage(
                                        "- The checks number cannot go bellow 0.", True
                                    )
                                    print()
                                    continue
                                task.SetTask(task.checks - checks)
                        ...
                        case _:
                            ...
                            print()
                            AttentionMessage(
                                f"- The task: {task.name} was marked as unfinished."
                            )
                            print()
                            ShowTasksInList()
            ...
            while True:
                ...
                while True:
                    ...
                    if user_choice == "N":
                        ...
                        task_lists[actual_list].tasks[index - 1].name = UserInput(
                            upper=False
                        )
                    ...
                    if user_choice == "C":
                        ...
                        match task.type:
                            case "S":
                                AttentionMessage(
                                    "- Single tasks does not have complements.", True
                                )
                                ShowTasksInList()
                                break
                            case "C":
                                ...
                                checks = UserInput(
                                    inte=True,
                                    positive=True,
                                    error_message="- Please, type a positive integer.",
                                )
        ...
        case "CLEAR":
            AttentionMessage(
                """- Are you sure that you want to delete all tasks from the actual list?
- If so, type [Y]. Else type anything""",
                True,
            )
            user_choice = UserInput()
        ...
        case "LIST":
            ...
            def ShowList():
                for index, lists in enumerate(task_lists, 1):
                    print()
                    print(
                        f"    {'*' if index - 1 == actual_list else ''}{index} ||| {lists.name}:"
                    )
                    ...
                    case "EDIT":
                        ...
                        while True:
                            ...
                            if 1 > index > len(task_lists):
                                ...
                                continue
                            print(
                                "- What should be the new name? (write nothing to cancel)."
                            )
                            name = UserInput(upper=False)
                            task_lists[actual_list].name = name
                ...
                case "A":
                    ...
                    with open(f"{name}.json", "w") as f:
                        ...
                        for lists in task_lists:
                            ...
                            save.append({"list_name": lists.name, "tasks": task_info})
                    ...
                    if type(data) == list:
                        for lists in data:
                            ...
                            for index, task in enumerate(lists["tasks"]):
                                tasks.append(
                                    pb.Task(
                                        task["type"], task["name"], task["complement"]
                                    )
                                )
...
- If you want a reminder of the commands every time you leave a Menu, type [remind].""",
                True,
            )
